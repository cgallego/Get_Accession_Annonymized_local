...
from dictionaries import program_loc, img_folder, my_aet, hostID, local_port, remote_aet, remote_IP, remote_port

# ******************************************************************************
print '''
-----------------------------------------------------------
Runs batch Exam list to get Dicom Exams on a local folder based on list on
Exam_list.txt(list of StudyIDs DicomExam# )
...
% June 14/14 -     getDICOMS_pacs to test new pushed studies, compulsory local Annonimization
-----------------------------------------------------------
'''
...
def getDICOMS_pacs(path_rootFolder, img_folder, remote_aet, remote_port, remote_IP, local_port, StudyID, AccessionN, countImages=False):
...
#          
    print '\n---- Begin query with ' + remote_aet + ' by StudyID ....' ;
    print "cmd -> " + cmd
...
if __name__ == '__main__':
    """
    usage:     python Pull_AccessionN_MRI_MARTEL.py 
    [INPUTS]
        Exam_list.txt (required) 
            text list with 2 columns:
            column1  column2  
            CADid    AccessionN
    """
    # Get Root folder ( the directory of the script being run)
    path_rootFolder = os.path.dirname(os.path.abspath(__file__))
    print"Pulling images to current directory, if different folder desired modify img_folder on dictionaries.py file"    
    print path_rootFolder
    
    if not (img_folder):
        img_folder = path_rootFolder
    
    if not os.path.exists('outcome'):
        os.makedirs('outcome')
            
    # Open filename list
    file_ids = open("Exam_list.txt","r")
    try:
        for fileline in file_ids:
            ...
            getDICOMS_pacs(path_rootFolder, img_folder, remote_aet, remote_port, remote_IP, local_port, StudyID, AccessionN, countImages=False)                        
